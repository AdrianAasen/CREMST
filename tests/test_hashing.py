...
class TestHash(unittest.TestCase):
    ...
    def test_downconvert(self):
        ...
        self.assertTrue(np.all(sf.downconvert_outcomes(qubit_index_1,test)==np.array([0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3])),"Downcovert does not mach ideal case.")
        qubit_index_2 = np.array([0,3])
        self.assertTrue(np.all(sf.downconvert_outcomes(qubit_index_2,test)==np.array([0,1,0,1,0,1,0,1,2,3,2,3,2,3,2,3])),"Downcovert does not mach ideal case.")
        
    def test_downconvert_false(self):
        # Test that 2 qubit downconvertion works
        test = np.arange(16)
        qubit_index_1 = np.array([0,2])
        self.assertFalse(np.all(sf.downconvert_outcomes(qubit_index_1,test)==np.array([0,0,1,1,2,2,3,3,0,0,1,1,2,2,3,3])), "Should be wrong")

    def test_downcovert_inverse_order(self):
        # Checks if inverse order matter
        test = np.arange(16)
        self.assertTrue(np.all(sf.downconvert_outcomes(np.array([1,0]),test) == sf.downconvert_outcomes(np.array([0,1]),test)),"Index order is not followed.")
    ...
    def test_single_calibration_states_generation(self):
        ...
        hash = np.array([1,0])
        test = sf.generate_calibration_states_from_hash(hash,one_qubit_calibration_states, 2)
    ...
    def test_higher_order_calibration_states(self):
        ...
        test = np.array([sf.generate_calibration_states_from_hash(function,one_qubit_calibration_states,2) for function in hash])
        ...
        hash = np.array([0,2])
        test = np.array(sf.generate_calibration_states_from_hash(hash,one_qubit_calibration_states,3))
        self.assertEqual(test[1,2,2],1)
        self.assertEqual(test[2,0,0],1)
